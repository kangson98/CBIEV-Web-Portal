<div class="card">
    <div class="card-header">
        iSpark Mentor Rcommendation Form - 
        @switch($type)
            @case(1)
                Dean/Head by
                @break
    ...
    <div class="card-body">
        @if ($type == 1)
            <form action="{{route('mentor.recommendation.dean.head.post')}}" method="post" >
        @endif
        @if ($type == 2)   
            <form action="{{route('mentor.recommendation.manager.post')}}" method="post" >
        @endif
            @csrf
            <input type="hidden" name="type" value="{{Crypt::encrypt($type)}}">
            <input type="hidden" name="recID" value="{{Crypt::encrypt($recID)}}">
            <div class="form-row">
                <div class="form-group col-sm-4">Do you recommend this person to become mentor?<span style="color:red"> *</span></div>
